...
// $Header: /home/cjm/cvs/ics_gui/create_dialog/CreateDialog.java,v 0.8 2000-11-24 11:36:50 cjm Exp $
import java.lang.*;
...
public class CreateDialog
{
	 ...
	 */
	public final static String RCSID = new String("$Id: CreateDialog.java,v 0.8 2000-11-24 11:36:50 cjm Exp $");
	/**
	...
	// CONFIG
		labelHashtable.put("config","CCD Config");
	// TELFOCUS
	...
	// headers and imports
		...
		outputStream.println("// Generated by "+this.getClass().getName()+" : Version : $Revision: 0.8 $");
	...
	// if there is a field of class ngat.phase2.nonpersist.NPInstrumentConfig - add some special methods here
		...
		// setConfigProperties method
			...
			outputStream.println("\t * @param ccdp The list of properties.");
			outputStream.println("\t */");
			outputStream.println("\tpublic void setConfigProperties(CcsCCDConfigProperties ccdp)");
			outputStream.println("\t{");
			outputStream.println("\t\tconfigProperties = ccdp;");
			outputStream.println("\t}");
			outputStream.println("");
	 ...
	 * as the message field's class and name. An exception is ngat.phase.InstrumentConfig,
	 * which uses class  ngat.phase.nonpersist.NPCCDConfig instead.
	...
	private void writeFieldDeclaration(Field f,PrintStream os)
	{
		...
		{
			os.println("\t/**");
			os.println("\t * Dialog field:"+f.getName()+
				" of type NPCCDConfig(a subclass of NPInstrumentConfig).");
			os.println("\t */");
			os.println("\tprivate NPCCDConfig "+f.getName()+";");
			os.println("\t/**");
			os.println("\t * "+f.getName()+"Properties is a list of configs to select from.");
			os.println("\t */");
			os.println("\tprivate CcsCCDConfigProperties "+f.getName()+"Properties;");
			configMethods = true;
		}
		else
		{
	...
	private void writeInputFieldGet(Field f,PrintStream os)
	{
		...
		// get nameId from textfield via nameProperties.getIdFromName (CcsCCDConfigProperties method)
			os.println("\t\t\ttry");
			os.println("\t\t\t{");
			...
			os.println("\t\t\t\treturn;");
			os.println("\t\t\t}");
		// construct NPCCDConfig from nameId via nameProperties.getCCDConfigById 
		// (CcsCCDConfigProperties method)
			os.println("\t\t\ttry");
			os.println("\t\t\t{");
			os.println("\t\t\t\t"+f.getName()+" = "+f.getName()+"Properties.getCCDConfigById("+
				f.getName()+"Id);");
			os.println("\t\t\t}");
			os.println("\t\t\tcatch(NumberFormatException e)");
			os.println("\t\t\t{");
