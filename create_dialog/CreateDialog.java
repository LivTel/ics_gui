...
// $Header: /home/cjm/cvs/ics_gui/create_dialog/CreateDialog.java,v 0.1 1999-11-22 09:51:30 cjm Exp $
import java.lang.*;
...
public class CreateDialog
{
	...
	// CONFIG
		labelHashtable.put("config","CCD Configuration");
	// TELFOCUS
	...
	// headers and imports
		...
		outputStream.println("// $Header: /home/cjm/cvs/ics_gui/create_dialog/CreateDialog.java,v 0.1 1999-11-22 09:51:30 cjm Exp $");
	...
	// RCSID
		outputStream.println("\t/**");
		outputStream.println("\t * Revision Control System id string, showing the version of the Class.");
		outputStream.println("\t */");
		outputStream.println("\tpublic final static String RCSID = new String(\"$Id: CreateDialog.java,v 0.1 1999-11-22 09:51:30 cjm Exp $\");");
	// for each field - define a field of the fieldname for temporary storage
		for(i=0;i<fieldList.size();i++)
		{
			f = (Field)(fieldList.elementAt(i));
			outputStream.println("\t/**");
			outputStream.println("\t * Dialog field:"+f.getName()+" of type "+
				getUnqualifiedClassName(f.getType().getName())+".");
			outputStream.println("\t */");
			outputStream.println("\t"+getUnqualifiedClassName(f.getType().getName())+" "+f.getName()+";");
		}
	...
	// end class, Log and finish
		outputStream.println("}");
		outputStream.println("//");
		outputStream.println("// $Log: not supported by cvs2svn $");
		outputStream.println("//");
	}
	...
	private void writeInputFieldDeclaration(Field f,PrintStream os)
	{
		...
		{
			os.println("\tJTextField "+f.getName()+"TextField;");
		}
		else if (f.getType().getName().equals("boolean"))
		{
			os.println("\tJCheckBox "+f.getName()+"CheckBox;");
		}
