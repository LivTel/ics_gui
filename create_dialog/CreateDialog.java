...
// $Header: /home/cjm/cvs/ics_gui/create_dialog/CreateDialog.java,v 0.5 2000-07-14 16:26:44 cjm Exp $
import java.lang.*;
...
public class CreateDialog
{
	 ...
	 */
	public final static String RCSID = new String("$Id: CreateDialog.java,v 0.5 2000-07-14 16:26:44 cjm Exp $");
	/**
	...
	// headers and imports
		...
		outputStream.println("// Generated by "+this.getClass().getName()+" : Version : $Revision: 0.5 $");
	 ...
	 * which uses class  ngat.phase.CCDConfig instead.
	...
	private void writeFieldDeclaration(Field f,PrintStream os)
	{
		if(f.getType().getName().equals("ngat.phase2.InstrumentConfig"))
		{
			...
			os.println("\t * Dialog field:"+f.getName()+
				" of type CCDConfig(a subclass of InstrumentConfig).");
			os.println("\t */");
			os.println("\tprivate CCDConfig "+f.getName()+";");
			os.println("\t/**");
	...
	private void writeInputFieldGet(Field f,PrintStream os)
	{
		...
		// get nameId from textfield via nameProperties.getIdFromName (CcsCCDConfigProperties method)
			...
			os.println("\t\t\t\treturn;");
			os.println("\t\t\t}");
		// construct CCDConfig from nameId via nameProperties.getCCDConfigById (CcsCCDConfigProperties method)
			os.println("\t\t\ttry");
			os.println("\t\t\t{");
