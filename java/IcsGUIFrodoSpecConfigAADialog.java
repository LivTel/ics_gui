...
*/
// IcsGUIFrodoSpecConfigAADialog.java
// $Header: /home/cjm/cvs/ics_gui/java/IcsGUIFrodoSpecConfigAADialog.java,v 1.2 2020-05-05 10:20:39 cjm Exp $
import java.lang.*;
import java.util.*;
import java.awt.*;
...
/**
 * This class provides an Add and Amend facility for FrodoSpec Configurations.
 * @author Chris Mottram
 * @version $Revision: 98404f49e4c35a5c59ab3678f31a3b1d35ba5297 $
 */
public class IcsGUIFrodoSpecConfigAADialog extends JDialog implements ActionListener
{
	/**
	 * Revision Control System id string, showing the version of the Class.
	 */
	public final static String RCSID = new String("$Id: IcsGUIFrodoSpecConfigAADialog.java | Tue May 5 10:20:41 2020 +0000 | Chris Mottram  $");
	/**
	 * Button height.
	 */
	...
	JComboBox armComboBox = null;
	JComboBox resolutionComboBox = null;
	 ...
	 */
	public IcsGUIFrodoSpecConfigAADialog(Frame owner,IcsGUIConfigProperties c)
	{
		super(owner,"Add/Amend FrodoSpec Configuration");
//		setResizable(false);
		configProperties = c;
	...
	// sub panel
		...
		subPanel.setLayout(new SizedGridLayout(0,2,new Dimension(DIALOG_WIDTH,FIELD_HEIGHT)));
	// arm
		label = new JLabel("Arm");
		subPanel.add(label);
		armComboBox = new JComboBox();
		armComboBox.setEditable(false);
		armComboBox.addItem("red");
		armComboBox.addItem("blue");
		subPanel.add(armComboBox);
	// sub panel
		subPanel = new JPanel();
		getContentPane().add(subPanel);
		subPanel.setLayout(new SizedGridLayout(0,2,new Dimension(DIALOG_WIDTH,FIELD_HEIGHT)));
	// resolution grating position
		...
		subPanel.add(resolutionComboBox);
	// sub panel
		subPanel = new JPanel();
		getContentPane().add(subPanel);
		subPanel.setLayout(new SizedGridLayout(0,2,new Dimension(DIALOG_WIDTH,FIELD_HEIGHT)));
	...
	public void add()
	{
		...
		armComboBox.setSelectedItem("red");
		resolutionComboBox.setSelectedItem("low");
	...
	public void amend(int id)
	{
		...
		armComboBox.setSelectedItem(configProperties.getConfigArmString(id));
		resolutionComboBox.setSelectedItem(configProperties.getConfigResolutionString(id));
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		int i;
		int xBin,yBin;
		int arm = FrodoSpecConfig.NO_ARM,resolution = FrodoSpecConfig.RESOLUTION_UNKNOWN;
		...
		// get and test numeric data from text fields
			try
			{
				armString = (String)(armComboBox.getSelectedItem());
				if(armString.equals("red"))
					arm = FrodoSpecConfig.RED_ARM;
				else if(armString.equals("blue"))
					arm = FrodoSpecConfig.BLUE_ARM;
				else if(armString.equals("unknown"))
					arm = FrodoSpecConfig.NO_ARM;
				resolutionString = (String)(resolutionComboBox.getSelectedItem());
				if(resolutionString.equals("low"))
					resolution = FrodoSpecConfig.RESOLUTION_LOW;
				else if(resolutionString.equals("high"))
					resolution = FrodoSpecConfig.RESOLUTION_HIGH;
				else if(resolutionString.equals("unknown"))
					resolution = FrodoSpecConfig.RESOLUTION_UNKNOWN;
		...
		// set data
			...
			configProperties.setConfigType(configId,IcsGUIConfigProperties.CONFIG_TYPE_SPECTROGRAPH_FRODOSPEC);
			...
			configProperties.setConfigArm(configId,arm);
			configProperties.setConfigResolution(configId,resolution);
...
// Revision 1.1  2008/03/05 14:34:37  cjm
// Initial revision
//
//
