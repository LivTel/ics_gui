...
public class IcsGUISpratConfigAADialog extends JDialog implements ActionListener
{
	 ...
	 */
	public final static String RCSID = new String("$Id: IcsGUISpratConfigAADialog.java | Tue May 5 10:20:41 2020 +0000 | Chris Mottram  $");
	/**
	 * Button height.
	 */
	...
	JComboBox slitPositionComboBox = null;
	JComboBox grismPositionComboBox = null;
	JComboBox grismRotationComboBox = null;
	 ...
	 */
	public IcsGUISpratConfigAADialog(Frame owner,IcsGUIConfigProperties c)
	{
		super(owner,"Add/Amend Sprat Configuration");
//		setResizable(false);
		configProperties = c;
	...
	// sub panel
		...
		subPanel.setLayout(new SizedGridLayout(0,2,new Dimension(DIALOG_WIDTH,FIELD_HEIGHT)));
	// slit position
		label = new JLabel("Slit Position");
		subPanel.add(label);
		slitPositionComboBox = new JComboBox();
		slitPositionComboBox.setEditable(false);
		slitPositionComboBox.addItem("in");
		slitPositionComboBox.addItem("out");
		subPanel.add(slitPositionComboBox);
	// sub panel
		subPanel = new JPanel();
		getContentPane().add(subPanel);
		subPanel.setLayout(new SizedGridLayout(0,2,new Dimension(DIALOG_WIDTH,FIELD_HEIGHT)));
	// grism position
		label = new JLabel("Grism Position");
		subPanel.add(label);
		grismPositionComboBox = new JComboBox();
		grismPositionComboBox.setEditable(false);
		grismPositionComboBox.addItem("in");
		grismPositionComboBox.addItem("out");
		subPanel.add(grismPositionComboBox);
	// sub panel
		subPanel = new JPanel();
		getContentPane().add(subPanel);
		subPanel.setLayout(new SizedGridLayout(0,2,new Dimension(DIALOG_WIDTH,FIELD_HEIGHT)));
	// grism rotation
		label = new JLabel("Grism Rotation");
		subPanel.add(label);
		grismRotationComboBox = new JComboBox();
		grismRotationComboBox.setEditable(false);
		grismRotationComboBox.addItem("0");
		grismRotationComboBox.addItem("1");
		subPanel.add(grismRotationComboBox);
	// sub panel
		subPanel = new JPanel();
		getContentPane().add(subPanel);
		subPanel.setLayout(new SizedGridLayout(0,2,new Dimension(DIALOG_WIDTH,FIELD_HEIGHT)));
	...
	public void add()
	{
		...
		slitPositionComboBox.setSelectedItem("out");
		grismPositionComboBox.setSelectedItem("out");
		grismRotationComboBox.setSelectedItem("0");
	...
	public void amend(int id)
	{
		...
		slitPositionComboBox.setSelectedItem(configProperties.getConfigSlitPositionString(id));
		grismPositionComboBox.setSelectedItem(configProperties.getConfigGrismPositionString(id));
		grismRotationComboBox.setSelectedItem(configProperties.getConfigGrismRotationString(id));
		for(int i=0;i<WINDOW_COUNT;i++)
		{
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		int testId = 0;
		int i;
		int xBin,yBin,windowFlags,slitPosition,grismPosition,grismRotation;
		...
		// get and test numeric data from text fields
			try
			{
				slitPositionString = (String)(slitPositionComboBox.getSelectedItem());
				if(slitPositionString.equals("in"))
					slitPosition = SpratConfig.POSITION_IN;
				else if(slitPositionString.equals("out"))
					slitPosition = SpratConfig.POSITION_OUT;
				else
					slitPosition = 0;
				grismPositionString = (String)(grismPositionComboBox.getSelectedItem());
				if(grismPositionString.equals("in"))
					grismPosition = SpratConfig.POSITION_IN;
				else if(grismPositionString.equals("out"))
					grismPosition = SpratConfig.POSITION_OUT;
				else
					grismPosition = 0;
				grismRotationString = (String)(grismRotationComboBox.getSelectedItem());
				if(grismRotationString.equals("0"))
					grismRotation = 0;
				else if(grismRotationString.equals("1"))
					grismRotation = 1;
				else
					grismRotation = -1;
		...
		// set data
			...
			configProperties.setConfigSlitPosition(configId,slitPosition);
			configProperties.setConfigGrismPosition(configId,grismPosition);
			configProperties.setConfigGrismRotation(configId,grismRotation);
